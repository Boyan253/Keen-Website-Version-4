...
class CMSDatabase {
  ...
  async getContent(section?: string): Promise<CMSContent[]> {
    ...
    return section ? allContent.filter(item => item.section === section) : allContent
  }
  ...
  async setContent(content: Omit<CMSContent, 'id' | 'createdAt' | 'updatedAt'>): Promise<CMSContent> {
    const id = `${content.section}-${content.key}`
    const now = new Date().toISOString()
    
    const newContent: CMSContent = {
      ...content,
      id,
      createdAt: this.content.has(id) ? this.content.get(id)!.createdAt : now,
      updatedAt: now
    }
  ...
  async getActiveColorScheme(): Promise<ColorScheme | null> {
    const schemes = Array.from(this.colorSchemes.values())
    return schemes.find(scheme => scheme.isActive) || null
  }

  async setColorScheme(scheme: Omit<ColorScheme, 'id' | 'createdAt' | 'updatedAt'>): Promise<ColorScheme> {
    const id = scheme.name.toLowerCase().replace(/\s+/g, '-')
    const now = new Date().toISOString()
    
    // If this is being set as active, deactivate others
    if (scheme.isActive) {
      for (const [key, existingScheme] of this.colorSchemes) {
        if (existingScheme.isActive) {
          existingScheme.isActive = false
          this.colorSchemes.set(key, existingScheme)
        }
      }
    }
    ...
    const newScheme: ColorScheme = {
      ...scheme,
      id,
      createdAt: this.colorSchemes.has(id) ? this.colorSchemes.get(id)!.createdAt : now,
      updatedAt: now
    }
    
    this.colorSchemes.set(id, newScheme)
    return newScheme
  }
  ...
  async getImages(section?: string): Promise<ImageAsset[]> {
    ...
    return section ? allImages.filter(img => img.section === section) : allImages
  }
  ...
  async getSection(section: string): Promise<SectionConfig | null> {
    return this.sections.get(section) || null
  }

  async setSection(section: Omit<SectionConfig, 'id' | 'createdAt' | 'updatedAt'>): Promise<SectionConfig> {
    const now = new Date().toISOString()
    
    const newSection: SectionConfig = {
      ...section,
      id: section.section,
      createdAt: this.sections.has(section.section) ? this.sections.get(section.section)!.createdAt : now,
      updatedAt: now
    }
    
    this.sections.set(section.section, newSection)
    return newSection
  }
  ...
  private async initializeDefaultContent(): Promise<void> {
    ...
    for (const content of defaultContent) {
      console.log('Setting content:', content.key, content.section)
...
export const cmsDatabase = new CMSDatabase()
